...
import inscribed_rec_imgMask_multiImgs_pool
# import utils
import pandas as pd
import sys,os
import glob
...
imgs_dir_roots=r'./data/results/imgs_segs'
subfolders=[f.path for f in os.scandir(imgs_dir_roots) if f.is_dir()]
...
rec_save_path='./data/results'
rec_path=os.path.join(rec_save_path,'recs_segs')
...
if __name__ == '__main__':
    for folder in tqdm(subfolders):
        fns=glob.glob(folder+"/*.{}".format(suffix))    
        
        img_rec_path=os.path.join(rec_path,Path(folder).stem)
        if not os.path.exists(img_rec_path):
        ...
        for img_fn in fns:  
            ...
            prod_args=partial(inscribed_rec_imgMask_multiImgs_pool.recs, args=[row_n,col_n,idx_nz_list])
            with Pool(8) as p:
            ...
            try:
                ...
                img_max_rec_pil.save(os.path.join(img_rec_path,'{}.jpg'.format(fn_stem)))    
            except:
                failed_img_fns.append(img_fn)